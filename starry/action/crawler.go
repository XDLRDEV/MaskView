...
/*func Crawler(c echo.Context) error {
	m := new(models.Copyright)
	if err := c.Bind(m); err != nil {
		return err
	}
	fmt.Printf("鲁棒水印信息为: %s，截屏文件地址为: %s，侵权网址为：%s \n", m.RobustInfo, m.ScreenShot, m.Url)
	txId := grpc.FindCopyrightClient(m)
	var id uint64
	if txId.Result {
		m.TxId = txId.TxId
		if primaryKey, err := gorm_mysql.CreateCopyright(m); err != nil {
			return err
		} else {
			id = primaryKey
		}
	} else {
		return c.JSON(http.StatusOK, models.ResultWithNote{Result: false, Note: "不用管"})
	}
	a, err := api.DecodeByRobustInfo(m.RobustInfo)
	if err != nil {
		return err
	}
	txId = grpc.AuthRightClient(a)
	if txId.Result {
		a.TxId = txId.TxId
		if err := gorm_mysql.CreateAuthRight(a); err != nil {
			return err
		}
		f := models.Confirm{
			Uid:            a.Uid,
			DataNumber:     a.DataNumber,
			DataType:       "picture",
			DataHash:       a.DataHash,
			ZeroInfo:       a.ZeroInfo,
			RobustInfo:     a.RobustInfo,
			ScreenShot:     m.ScreenShot,
			Url:            m.Url,
			ConfirmMessage: a.ConfirmMessage,
			ConfirmTime:    a.ConfirmTime,
		}
		txId = grpc.ConfirmClient(&f)
		if txId.Result {
			f.TxId = txId.TxId
			if err := gorm_mysql.CreateConfirm(&f); err != nil {
				return err
			}
			if err = gorm_mysql.UpdateCopyrightCode(&id); err != nil {
				return err
			}
		} else {
			return c.JSON(http.StatusOK, models.ResultWithNote{Result: false, Note: "不用管"})
	...
	return c.JSON(http.StatusOK, models.ResultWithNote{Result: true, Note: "不用管"})
