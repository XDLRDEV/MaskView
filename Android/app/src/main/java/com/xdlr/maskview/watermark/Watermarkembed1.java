...
public class Watermarkembed1 {
    ...
    public static Watermarkembed1 waterMarkEmbed1(int nLevels, String biort, String qShift, Bitmap bitmap, int rr, int cc, int a, int b, Complex[][] markImg, double[][] Ug, double minradies, int pattern) throws IOException {
        ...
        for (int i=0;i<a;++i){
            for (int j=0;j<b;++j){
                int[] x=new int[4];
                int[] y=new int[4];
                x[0]=t1[i][j][0];y[0]=t1[i][j][1];
                x[1]=t2[i][j][0];y[1]=t2[i][j][1];
                x[2]=t3[i][j][0];y[2]=t3[i][j][1];
                x[3]=t4[i][j][0];y[3]=t4[i][j][1];
                for (int l=0;l<4;++l){
