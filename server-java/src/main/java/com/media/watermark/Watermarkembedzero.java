...
public class Watermarkembedzero {
    private double[] binaryFeature;                                                                                     //Zero watermark feature sequence
    ...
    /**
     * @param matrix Initial input 2-D matrix
     *@return  this Zero watermark feature sequence
     */
    public static Watermarkembedzero watermarkembedzero(double[][] matrix){
        ...
        int chf_len=36;
        int bpf_num=64;
        double[] base_wm={0,1,1,0,1,0,0,0,1,0};                                                                         //The original code
        int num=chf_len*2+bpf_num;                                                                                      //Zero watermark sequence length
        double[] wm=Wm_gen.wm_gen(base_wm,num).getWm();                                                                 //Zero watermark sequence length
        ...
        double[][] Impatch=new double[mr/2][mc/2];                                                                      //characteristic matrix
        //Divide the matrix into small blocks of 2*2 and calculate the
        // average value to generate the characteristic matrix
        ...
        //Converts the feature matrix into a bitmap image
        BitImgGen bitImg= BitImgGen.BitImgGen(Impatch);
        GenCHF genCHF=new GenCHF(Impatch,bitImg.getMax_quan(),bitImg.getMin_quan(),init_code_book_chf,chf_len);         //Color histogram feature
        GenBPF genBPF=new GenBPF(bitImg.getBit_img(),init_code_book_bpf,bpf_cbb,bpf_cbs);                               //Bit pattern feature

        ArrayList<int[][]> gen=genCHF.GenCHF1();                                                                        //Color histogram feature sequence
        double[][] bpf=genBPF.GenBPF1();                                                                                //Bit pattern feature sequence

        int genRow=gen.get(0).length;
        int genCole=gen.get(0)[0].length;
        double[][] CHF_max=new double[genRow][genCole];                                                                 //Maximum quantizer feature sequence
        double[][] CHF_min=new double[genRow][genCole];                                                                 //Minimum quantizer feature sequence
        for (int i=0;i<genRow;++i){
            for (int j=0;j<genCole;++j){
                CHF_max[i][j]=gen.get(0)[i][j];
                CHF_min[i][j]=gen.get(1)[i][j];
            }
        }

        double[][] BPF=bpf;                                                                                             //Sequence of bit-mode diagram features
        //Merged feature sequence
        Genbinaryfeature genbinaryfeature=Genbinaryfeature.genbinaryFeature(CHF_max,CHF_min,BPF,bpf_num,chf_len);
        double[] feature=genbinaryfeature.getBinaryFeature();                                                           //The secret key sequence
        //The feature sequence and the codebook are exclusive or secret key sequences are generated
